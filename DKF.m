function [ x_estimated ] = DKF ( x, Time, phi, H, Z, x_hat0_min, P0_min, R, Q, Plot )
%{ 
      ...
      and variance of Process model noise matrix
...
%% function body
        ...
        for i=1:length(Z)
            %% Calculate the gain
            K{i}=P_min{i}*H'*inv(H*P_min{i}*H'+R);
            %% Update estimate
            x_hat_p{i}=x_hat_min{i}+K{i}*(Z(i)-H*x_hat_min{i});
            %% Update error
            P_p{i}=(eye(length(H(1,:)))-K{i}*H)*P_min{i};
            %% Project ahead
...
if Plot(1) == 1
    ...
    for i=1:length(Z(:,2))
        ...
        plot(Time,x(i,:));
        plot(Time(2:length(Z)),Z(i,2:length(Z)),'--');
        grid on;
    ...
    for i=1:length(Z(:,2))
        ...
        plot(Time(2:length(Z)),100*abs((Z(i,2:length(Z))-x(i,2:length(Z))))./x(i,2:length(Z)));
        grid on;
        ylabel(['error'  num2str(i) '%'],'fontsize',18)
        legend({['error'  num2str(i) '% with \mu = ' num2str(mean(100*abs((Z(i,2:length(Z))-x(i,2:length(Z)))./x(i,2:length(Z))))) ', \sigma = ' num2str(std(100*abs((Z(i,2:length(Z))-x(i,2:length(Z)))./x(i,2:length(Z)))))] });
    end
    if i > 1
        subplot(length(x(:,1)),1,i);
        xlabel('Time (sec)','fontsize',18)
    ...
    for i=1:length(Z(:,2))
        ...
        plot(Time,x(i,:));
        plot(Time(2:length(Z)),Z(i,2:length(Z)),'--');
        plot(Time(2:length(Z)),x_estimated(i,2:length(Z)),'-.');
        grid on;
    ...
    for i=1:length(x(:,2))
        ...
        plot(Time(2:length(Z)),100*abs((x_estimated(i,2:length(Z))-x(i,2:length(Z)))./x(i,2:length(Z))));
        grid on;
        ylabel(['error' num2str(i) '%'],'fontsize',18)
        legend({['error'  num2str(i) '% with \mu = ' num2str(mean(100*abs((x_estimated(i,5:length(Z))-x(i,5:length(Z)))./x(i,5:length(Z))))) ', \sigma = '  num2str(std(100*abs((x_estimated(i,5:length(Z))-x(i,5:length(Z)))./x(i,5:length(Z)))))] });
    end
    if i > 1
        subplot(length(x(:,1)),1,i);
        xlabel('Time (sec)','fontsize',18)
